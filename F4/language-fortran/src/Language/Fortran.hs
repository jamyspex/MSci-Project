...
instance Show SrcLoc
  ...
  show (SrcLoc f l c) =
    "{" ++ f ++ ", line = " ++ show l ++ ", col = " ++ show c ++ "}"
  show NoSrcLoc = "No source location"
...
data ProgUnit p
  ...
  | PSeq p
         SrcSpan
         (ProgUnit p)
         (ProgUnit p) -- sequence of programs
  | Prog p
         SrcSpan
         (ProgUnit p) -- useful for {#p: #q : program ... }
  | NullProg p
             SrcSpan -- null
  | IncludeProg p
                SrcSpan
...
data Uses p
  = Use p
        ...
        p
  | UseNil p
  deriving (Show, Functor, Typeable, Data, Eq)--             use's        implicit              decls  stmts
...
data Decl p
  = Decl p
         ...
         (Type p) -- declaration stmt
  | Namelist p
             [(Expr p, [Expr p])] -- namelist declaration
  | DataDecl p
             (DataForm p)
  | Equivalence p
                SrcSpan
  ...
  | AccessStmt p
               ...
               [GSpec p] -- access stmt
  | ExternalStmt p
                 [String] -- external stmt
  | Interface p
              (Maybe (GSpec p))
              [InterfaceSpec p] -- interface declaration
  | Common p
           SrcSpan
           (Maybe String)
           [Expr p]
  ...
  | DSeq p
         (Decl p)
         (Decl p) -- list of decls
  | TextDecl p
             String -- cpp switches to carry over
  | NullDecl p
             SrcSpan -- null
                -- units-of-measure extension
  | MeasureUnitDef p
                   SrcSpan
                   [(MeasureUnit, MeasureUnitSpec p)]
  deriving (Show, Functor, Typeable, Data, Eq)
...
data DataForm p =
  Data p
       [(Expr p, Expr p)]
  deriving (Show, Functor, Typeable, Data, Eq) -- data declaration
...
data Fortran p
  ...
  | Forall p
           SrcSpan
           ([(String, (Expr p), (Expr p), (Expr p))], (Expr p))
           (Fortran p)
  | Goto p
         SrcSpan
         String
  ...
  | SelectStmt p
               ...
               (Expr p)
               [((Expr p), (Fortran p))]
               (Maybe (Fortran p)) -- GAV ADDED
  | OpenCLMap p
              SrcSpan -- Node to represent the data needed for an OpenCL map kernel
...
data Expr p
  ...
  | ConS p
         SrcSpan
         String -- String representing a constant
  | ParenthesizedExpr p
                      SrcSpan
                      (Expr p)
  | Var p
        SrcSpan
        [(VarName p, [Expr p])] -- WV: the [Expr  p] list is used for array accesses
  | Bin p
        SrcSpan
        (BinOp p)
        (Expr p)
  ...
  | Sqrt p
         SrcSpan
         (Expr p) -- WV: This is silly, should be handled either for all intrinsics or none
  | ArrayCon p
             SrcSpan
             [(Expr p)]
  | AssgExpr p
             SrcSpan
             String
             (Expr p) -- WV: This is for use in e.g. parameter declarations I guess
  deriving (Show, Functor, Typeable, Data, Eq)
