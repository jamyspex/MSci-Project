...
-- all call statments in currently discovered files
      ...
      otherSubsParseResults <- findOtherRequiredSubs parseCurried
                                                     sraWithSubCalls
      ...
      let sra' = populateSubCalls opts $ SRA
...
constructSubroutineTable opts sra = DMap.fromList
  $ map (\subname -> (subname, constructOneSubRec subname opts sra)) subnames
  where subnames = map (\(key, _) -> key) $ DMap.toList (subroutineToFile sra)


constructOneSubRec :: String -> F4Opts -> SubRecAnalysis -> SubRec
constructOneSubRec subname opts sra = MkSubRec ast
                                               filename
                                               lines
                                               subname
                                               argTrans
...
populateArgTrans sra = sra
  { subroutineToArgTrans = DMap.fromList forConversionToArgTransMap
  }
 where
...
buildArgTransMapValue argNames varNames = map
  (\(arg, var) -> ArgTrans arg var)
  matched
  where matched = zip argNames varNames
...
populateSubCalls opts sra = sra
  { subroutineToCalls = DMap.fromList subnamesToCallsMap
  }
 where
  ...
  callsOfInterest = map
    (\(subname, calls) -> (subname, filter (checkIfSubOfInterest opts) calls))
    callsInFiles
  subnamesToCallsMap =
    trace
        (  "callsOfInterest: "
...
extractProgUnitName ast
  ...
  where subNames = everything (++) (mkQ [] getSubNames) ast
...
replaceKernels kernelPairs subroutine = foldl
...
checkParseIntermediatariesHaveCorrectKeys sra = if (foldr (&&) True allResults)
  then Success
