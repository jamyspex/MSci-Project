...
for :: String -> Int -> Expr Anno -> Fortran Anno -> Fortran Anno
for loopVar initial lhs =
  For nullAnno nullSrcSpan (varName loopVar) (con initial) lhs (con 1)
...
argNode names = Arg nullAnno
                    (buildAstSeq (ASeq nullAnno) (NullArg nullAnno) argNames)
                    nullSrcSpan
  where argNames = map (ArgName nullAnno) names
...
fortranModule name = Module nullAnno
                            nullSrcSpan
