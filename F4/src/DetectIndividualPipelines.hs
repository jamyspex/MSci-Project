...
getTopLevelBlockStatements mergedBody
  -- mapM_
  --   (\nest ->
  ...
  where
    ...
    descendLoopNest current (FSeq _ _ f1 f2) =
      case result1Larger of
        ...
        _          -> result1
      where
        result1@(_, nl1, ic1) = descendLoopNest current f1 -- FIXME this probably needs to do something with the other FSeq branch
    ...
    descendLoopNest (name, nestingLevel, totalIterations) (For _ _ _ initExpr upbExpr _ body) =
      descendLoopNest
        ...
        --     " lwbInt = " ++ show lwbInt ++ " upbInt = " ++ show upbInt)
