...
buildSmartCacheForKernel k = do
 ...
 where
  requiredStencilStreams = filter (\s -> isTransit s || isStencil s) $ inputs k
  ...
  smartCacheItems =
    map (buildSmartCacheItem k numberOfStreamDimensions) requiredStencilStreams
...
buildStream DummySmartCacheItem {..} = map
  ...
  cacheItem =
    case
        calculateSmartCacheDetailsForStream
          ...
          inStream
      of
        SmartCacheDetailsForStream {..} -> SmartCacheItem
          { size                            = requiredBufferSize
          , inputStream                     = convertStencilStream inStream
          , maxPositiveOffset               = maxPosOffset
          , maxNegativeOffset               = maxNegOffset
          ...
          in  DummySmartCacheItem
                ...
                , outputStreamNames = map
                  (\stenIndices -> arrayName ++ concat
                    (imap
                      (\pos idx ->
                        "_" ++ case DMap.lookup pos loopVarPositionMap of
                          Just val -> val
  ...
  pointsAndVarNames = foldl
    (\acc (point, buffIndex) ->
      ( trace
          ...
          ++ " map = "
          ++ show (DMap.toList bodgedMapForLES)
          ++ " point = "
          ++ show point
          )
