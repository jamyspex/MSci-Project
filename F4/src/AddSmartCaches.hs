...
buildSmartCacheForKernel k = do
  ...
  where
    ...
    withInputStreamsUpdated = updateKernelInputStreams paddedSmartCacheItems k
...
updateKernelInputStreams :: [SmartCacheItem] -> Kernel -> Kernel
updateKernelInputStreams smartCacheItems kernel =
  kernel {inputs = nonStencilInputs ++ smartCacheOutputStreams}
  where
    nonStencilInputs = filter (not . isStencil) $ inputs kernel
    smartCacheOutputStreams = concatMap buildStream smartCacheItems
