...
parseFile cppDArgs cppXArgs fixedForm dir filename = do
  (preproc_inp, stash, moduleVarTable) <- preProcessingHelper cppDArgs
                                                              cppXArgs
                                                              fixedForm
                                                              True
                                                              dir
...
runCpp cppDArgs cppXArgs fixedForm dir filename = do
  (preproc_inp, _, _) <- preProcessingHelper cppDArgs
                                             cppXArgs
                                             fixedForm
                                             False
                                             dir
...
preProcessingHelper cppDArgs cppXArgs fixedForm inlineModules dir filename = do
  let filePrefix = case dir of
        "" -> "./"
        _  -> dir ++ "/"
      dFlagList = if not (null cppDArgs)
        then foldl (\accum item -> accum ++ ["-D" ++ item]) [] cppDArgs
        else []
      dFlagStr = if null dFlagList then "" else unwords dFlagList
  inp <- readFile (filePrefix ++ filename)
  let filename_no_dot | head filename == '.' = tail $ tail filename
                      | otherwise            = filename
  ...
  (exp_inp_lines', moduleVarTable) <- inlineDeclsFromUsedModules
    dir
    True
    exp_inp_lines''
    cppDArgs
...
outputExprFormatting (Var _ _ list) = foldl
  (++)
  ""
  (map
    (\(varname, exprList) ->
...
outputExprFormatting (Con _ _ str) =
  if takeLast 2 str == ".0" then take ((length str) - 2) str else str
...
listExtractSingleAppearances' (x : []) wholeList = if appearances == 1
  then [x]
  else []
  where appearances = listCountAppearances x wholeList
listExtractSingleAppearances' (x : tailList) wholeList = if appearances == 1
...
listRemoveDuplications a = foldl
  (\accum item -> if notElem item accum then accum ++ [item] else accum)
  []
  a

listConcatUnique :: Eq a => [a] -> [a] -> [a]
listConcatUnique a b = foldl
  (\accum item -> if notElem item accum then accum ++ [item] else accum)
  b
  a
...
extractContainedOperands expr = foldl
  (\accum item -> accum ++ (extractOperands item))
  []
  containedVars
  where containedVars = extractContainedVars expr
...
extractAssigneeFromDecl (Decl anno src lst typ) = head
  (extractVarNames assignee)
  where assignee = (\(x, _, _) -> x) (head lst)
...
shiftSrcSpanLineGlobal inc codeSeg =
  everywhere (mkT (shiftSrcSpan inc)) codeSeg
...
stretchSrc inc (srcLoc, (SrcLoc fe le ce)) =
  (srcLoc, (SrcLoc fe (le + inc) ce))
...
normaliseSrcSpan correctSpanned badSpanned = shiftSrcSpanLineGlobal
  lineDifference
  badSpanned
 where
...
hasOperand container contains = all (== True) $ map
...
replaceAllOccurences_varnamePairs codeSeg originals replacements = foldl
  (\accum (v1, v2) -> replaceAllOccurences_varname accum v1 v2)
  codeSeg
  pairs
  where pairs = zip originals replacements
...
getEarliestSrcSpan spans = Just
  (foldl
    (\accum item -> if checkSrcSpanBefore item accum then item else accum)
    (spans !! 0)
    spans
  )
...
getLatestSrcSpan spans = Just
  (foldl
    (\accum item -> if checkSrcSpanBefore item accum then accum else item)
    (spans !! 0)
    spans
  )
...
getEarliestSrcLoc locs = Just
  (foldl (\accum item -> if checkSrcLocBefore item accum then item else accum)
         (locs !! 0)
         locs
  )
...
extractPrimaryReductionOp assignee (Bin _ _ op expr1 expr2) =
 ...
 where
  ...
  childOp    = case primaryOp1 of
    ...
    Nothing -> primaryOp2
  assigneePresent =
    applyGeneratedSrcSpans expr1
      == applyGeneratedSrcSpans assignee
      || applyGeneratedSrcSpans expr2
      == applyGeneratedSrcSpans assignee
extractPrimaryReductionOp assignee assignment = Nothing

extractPrimaryReductionFunction :: Expr Anno -> Expr Anno -> String
extractPrimaryReductionFunction assignee (Var _ _ list) = foldl
  assigneePresent
  ""
  standardisedList
 where
    ...
    Just start -> case endInt of
      ...
      Just end -> case stepInt of
        ...
        Just step -> Just
          (map
            (fromIntegral)
            [round start :: Int, (round start :: Int) + (round step :: Int) .. round
              end :: Int]
          )
...
evaluateExpr_type vt (Var p src lst)
  | varString == "mod" = maybeBinOp_integral (evaluateExpr_type vt expr1)
...
evaluateExpr_type _ (Con _ _ str)
  ...
  | last str == '.' = Just
    ((read $ take (length str - 1) str) :: Float, Real nullAnno)
...
split delim str =
  map (\w -> map (\c -> if c == delim then ' ' else c) w) $ words
    (map (\c -> (if delim == c then ' ' else if c == ' ' then delim else c)) str
    )
...
findDeclLineVars line =
  if Data.List.isInfixOf "::" line && head (filter (/= ' ') line) /= '!'
    then
      let line_no_comments = head $ splitDelim "!" line -- (warning line ("<OL:"++line++">"))
...
isUseDecl line =
  ...
  in  if ((not (null chunks)) && (head chunks == "use")) then True else False -- init (tail (chunks !! 1)) else ""
...
readUsedModuleDecls sourceDir line cppDArgs cppXArgs fixedForm
  = let
      ...
      file_name_root
        | take (length "module_") module_name == "module_" = drop
          (length "module_")
          module_name
      ...
      do
       ...
       -- FIXME: I am simply ignoring stash and moduleVarTable for now!
        (module_content_str, stash, moduleVarTable) <- if test1
          then preProcessingHelper cppDArgs
                                   cppXArgs
                                   fixedForm
                                   True
                                   sourceDir
...
isDeclOnly module_content_str =
  ...
  in
    null non_decl_lines-- (warning (null non_decl_lines) (show non_decl_lines) )
...
addImplicitNone contentLines
  = let
      (beforeImplicitNoneDecl, afterImplicitNoneDecl, _) = foldl
        (\(bls, als, beforeDecl) line ->
          let beforeDecl' = if beforeDecl && (Data.List.isInfixOf "::" line)
                then False
                else beforeDecl
          in  if beforeDecl'
        ...
        ([], [], True)
        contentLines
    in  beforeImplicitNoneDecl
        ++ ["      implicit none"]
        ++ afterImplicitNoneDecl

splitDelim :: String -> String -> [String]
splitDelim patt line =
  let (chunks, last_chunk, _) = foldl
        (\(chunks, current_chunk, rest_of_line) ch ->
          if Data.List.isPrefixOf patt rest_of_line
            then
              ...
              then
                let ch' : rest_of_line' = rest_of_line
...
oneVarDeclPerVarDeclLine contentLines =
  let contentLines' :: [[String]]
      contentLines' = map
        (\line -> if isVarDeclWithMultipleVars line
...
getIterLoopVars (OpenCLReduce _ _ vrs vws lvars ilvars rvarexprs stmt1) =
  ilvars
getIterLoopVars _ = []
