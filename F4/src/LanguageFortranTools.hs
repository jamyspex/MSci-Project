...
parseFile ::
  ...
  -> String
  -> String
  -> IO ((Program Anno, [String]), (String, CodeStash), ModuleVarsTable)
parseFile cppDArgs cppXArgs fixedForm dir filename = do
  (preproc_inp, stash, moduleVarTable) <-
    preProcessingHelper cppDArgs cppXArgs fixedForm True dir filename
...
runCpp cppDArgs cppXArgs fixedForm dir filename = do
  (preproc_inp, _, _) <-
    preProcessingHelper cppDArgs cppXArgs fixedForm False dir filename
  return preproc_inp
...
preProcessingHelper ::
  ...
  -> String
  -> String
  -> IO (String, CodeStash, ModuleVarsTable)
preProcessingHelper cppDArgs cppXArgs fixedForm inlineModules dir filename = do
  ...
  let contentLines = lines inp
  -- Then we split out var decls into one per line
  let inp' = unlines contentLines -- $ oneVarDeclPerVarDeclLine contentLines
  ...
  let cpp_cmd =
        "cpp -Wno-invalid-pp-token -P " ++
        dFlagStr ++
        " " ++ filePrefix ++ filename_noext ++ "_tmp.f95 | grep -v -E '^\\s*$' "
  -- putStrLn cpp_cmd
  putStrLn cpp_cmd
  ...
  let exp_inp_lines_no_comments = map (takeWhile (/= '!')) (lines preproc_inp')
  ...
  (exp_inp_lines', moduleVarTable) <-
    inlineDeclsFromUsedModules
      ...
      cppDArgs
      cppXArgs
      fixedForm -- FIXME: should this not be inlineModules instead of True?
  let preproc_inp'' = unlines exp_inp_lines'
...
errorLocationFormatting (SrcLoc filename line column, srcEnd) =
  show line ++ ":" ++ show column --"line " ++ show line ++ ", column " ++ show column

errorLocationRangeFormatting :: SrcSpan -> String
errorLocationRangeFormatting ((SrcLoc _ line_start _), (SrcLoc _ line_end _)) =
  "line " ++ show line_start ++ " and line " ++ show line_end -- ++ ", column " ++ show column
...
outputExprFormatting (Bin _ _ op expr1 expr2) =
  "(" ++
  outputExprFormatting expr1 ++
  " " ++ op_str ++ " " ++ outputExprFormatting expr2 ++ ")"
  where
    op_str =
      case op of
        Plus p   -> "+"
...
outputExprFormatting (Unary _ _ unOp expr) =
  "(" ++ op_str ++ outputExprFormatting expr ++ ")"
  where
    op_str =
      case unOp of
        UMinus p -> "-"
...
listExtractSingleAppearances' (x:[]) wholeList =
  if appearances == 1
    then [x]
    else []
  where
    appearances = listCountAppearances x wholeList
listExtractSingleAppearances' (x:tailList) wholeList =
  if appearances == 1
    then [x] ++ listExtractSingleAppearances' tailList wholeList
    else listExtractSingleAppearances' tailList wholeList
  where
    appearances = listCountAppearances x wholeList
...
listCountAppearances value (x:[])
  | value == x = 1
  | otherwise = 0
listCountAppearances value (x:list)
  | value == x = 1 + listCountAppearances value list
...
listRemoveDuplications a =
  foldl
    (\accum item ->
       if notElem item accum
         then accum ++ [item]
         else accum)
    []
    a
...
extractArgName codeSeg =
  case codeSeg of
    ArgName _ _ -> [codeSeg]
    _           -> []

extractReductionVarNames :: Fortran Anno -> [VarName Anno]
extractReductionVarNames (OpenCLReduce _ _ _ _ _ _ redVars _) =
  map (fst) redVars -- WV20170426
...
extractOperands (Bin _ _ _ expr1 expr2) =
  extractOperands expr1 ++ extractOperands expr2
extractOperands (Unary _ _ _ expr) = [expr]
extractOperands (ESeq _ _ expr1 expr2) =
  extractOperands expr1 ++ extractOperands expr2
extractOperands (Bound _ _ expr1 expr2) =
  extractOperands expr1 ++ extractOperands expr2
...
extractVarNames (Var _ _ lst) =
  let vs' =
        filter
          (\((VarName _ v), args) ->
             not (v `elem` f95IntrinsicFunctions && length args > 0))
          lst
      vs'' = map (\(x, _) -> x) vs'
   in vs'' -- if length vs'' == 0 then [VarName nullAnno "DUMMY"] else vs''
extractVarNames _ = []

extractMaybeVarNames :: Expr Anno -> Maybe [VarName Anno]
extractMaybeVarNames (Var _ _ lst) =
  let vs' =
        filter
          (\((VarName _ v), args) ->
             not (v `elem` f95IntrinsicFunctions && length args > 0))
          lst
      vs'' = map (\(x, _) -> x) vs'
   in Just vs'' -- if length vs'' == 0 then [VarName nullAnno "DUMMY"] else vs''
...
extractContainedVars (Var _ _ lst) =
  foldl (\accumExprs (itemVar, itemExprs) -> accumExprs ++ itemExprs) [] lst
extractContainedVars _ = []

-- WV
extractContainedVarsWV :: (Typeable p, Data p) => Expr p -> [Expr p]
extractContainedVarsWV expr =
  case expr of
    (Var _ _ lst) ->
      let exprs =
            foldl
              (\accumExprs (itemVar, itemExprs) -> accumExprs ++ itemExprs)
              []
              lst
       in foldl (++) exprs (map extractContainedVarsWV exprs)
    _ -> []

extractContainedOperands :: (Typeable p, Data p) => Expr p -> [Expr p]
extractContainedOperands expr =
  foldl (\accum item -> accum ++ (extractOperands item)) [] containedVars
  where
    containedVars = extractContainedVars expr

extractAssignments :: Fortran Anno -> [Fortran Anno]
extractAssignments codeSeg =
  case codeSeg of
...
extractVarNames_loopVars ::
     [(VarName Anno, Expr Anno, Expr Anno, Expr Anno)] -> [VarName Anno]
...
generateSrcSpan [] ((SrcLoc sFile sLine sCol), (SrcLoc eFile eLine eCol)) =
  ( SrcLoc {srcFilename = "generated", srcLine = sLine, srcColumn = sCol}
  , SrcLoc {srcFilename = "generated", srcLine = eLine, srcColumn = eCol})
generateSrcSpan filename ((SrcLoc sFile sLine sCol), (SrcLoc eFile eLine eCol)) =
  ( SrcLoc {srcFilename = filename, srcLine = sLine, srcColumn = sCol}
  , SrcLoc {srcFilename = filename, srcLine = eLine, srcColumn = eCol})

applySrcSpan :: SrcSpan -> Fortran Anno -> Fortran Anno
applySrcSpan src (OpenCLMap anno _ read written loopvs iterloopvs fortran) =
  OpenCLMap anno src read written loopvs iterloopvs fortran -- WV20170426
applySrcSpan src (OpenCLReduce anno _ read written loopvs iterloopvs rvs fortran) =
  OpenCLReduce anno src read written loopvs iterloopvs rvs fortran -- WV20170426
applySrcSpan src _ = error "applySrcSpan: unnsupported AST node"

applyGlobalSrcSpan :: (Data (a Anno)) => SrcSpan -> a Anno -> a Anno
applyGlobalSrcSpan srcSpan codeSeg =
  everywhere (mkT (replaceSrSpan srcSpan)) codeSeg
...
shiftSrcSpan inc ((SrcLoc fs ls cs), (SrcLoc fe le ce)) =
  ((SrcLoc fs (ls + inc) cs), (SrcLoc fe (le + inc) ce))
...
normaliseSrcSpan correctSpanned badSpanned =
  shiftSrcSpanLineGlobal lineDifference badSpanned
  where
...
srcSpanInSrcSpanRange start finish inside =
  srcSpanInSrcSpan (fst start, snd finish) inside

srcSpanInSrcSpan :: SrcSpan -> SrcSpan -> Bool
srcSpanInSrcSpan ((SrcLoc _ rSLine rSCol), (SrcLoc _ rELine rECol)) ((SrcLoc _ srcSLine srcSCol), (SrcLoc _ srcELine srcECol)) =
  startAfterRange && endsBeforeRange
  where
    startAfterRange =
      srcSLine > rSLine || srcSLine == rSLine && srcSCol >= rSCol
    endsBeforeRange =
      srcELine < rELine || srcELine == rELine && srcECol <= rECol
...
generateLTExpr expr1 expr2 =
  Bin nullAnno nullSrcSpan (RelLT nullAnno) expr1 expr2

generateAndExpr :: Expr Anno -> Expr Anno -> Expr Anno
generateAndExpr expr1 expr2 =
  Bin nullAnno nullSrcSpan (And nullAnno) expr1 expr2
...
generateAdditionExpr expr1 expr2 =
  Bin nullAnno nullSrcSpan (Plus nullAnno) expr1 expr2

generateProductExpr :: Expr Anno -> Expr Anno -> Expr Anno
generateProductExpr expr1 expr2 =
  Bin nullAnno nullSrcSpan (Mul nullAnno) expr1 expr2

generateSubtractionExpr :: Expr Anno -> Expr Anno -> Expr Anno
generateSubtractionExpr expr1 expr2 =
  Bin nullAnno nullSrcSpan (Minus nullAnno) expr1 expr2

generateDivisionExpr :: Expr Anno -> Expr Anno -> Expr Anno
generateDivisionExpr expr1 expr2 =
  Bin nullAnno nullSrcSpan (Div nullAnno) expr1 expr2
...
hasOperand container contains =
  all (== True) $
  map
    (\x -> elem x (extractOperands $ applyGeneratedSrcSpans container))
    (extractOperands $ applyGeneratedSrcSpans contains)
...
appendAnnotation original key appendage =
  case original of
    For anno f2 f3 f4 f5 f6 f7 ->
      For (appendToMap key appendage anno) f2 f3 f4 f5 f6 f7
    OpenCLMap anno f2 f3 f4 f5 f6 f7 ->
      OpenCLMap (appendToMap key appendage anno) f2 f3 f4 f5 f6 f7 -- WV20170426
    OpenCLReduce anno f2 f3 f4 f5 f6 f7 f8 ->
      OpenCLReduce (appendToMap key appendage anno) f2 f3 f4 f5 f6 f7 f8 -- WV20170426
    _ -> original
        -- appendToMap

appendAnnotationList :: Fortran Anno -> String -> [String] -> Fortran Anno
appendAnnotationList original key appendage =
  foldl (\accum item -> appendAnnotation accum key item) original appendage

appendAnnotationMap :: Fortran Anno -> Anno -> Fortran Anno
appendAnnotationMap codeSeg newMap =
  case codeSeg of
    For anno f2 f3 f4 f5 f6 f7 ->
      For (combineAnnotations newMap anno) f2 f3 f4 f5 f6 f7
    OpenCLMap anno f2 f3 f4 f5 f6 f7 ->
      OpenCLMap (combineAnnotations newMap anno) f2 f3 f4 f5 f6 f7 -- WV20170426
    OpenCLReduce anno f2 f3 f4 f5 f6 f7 f8 ->
      OpenCLReduce (combineAnnotations newMap anno) f2 f3 f4 f5 f6 f7 f8 -- WV20170426
...
removeAnnotations original =
  case original of
    ...
    OpenCLReduce anno f2 f3 f4 f5 f6 f7 f8 ->
      OpenCLReduce nullAnno f2 f3 f4 f5 f6 f7 f8 -- WV20170426
    _ -> original
...
usesVarName_list varNames (Var _ _ list) =
  foldl (||) False $ map (\(varname, exprs) -> elem varname varNames) list -- so any variable used as an array index or argument of a function call is ignored!
usesVarName_list varNames (Bin _ _ _ expr1 expr2) =
  (usesVarName_list varNames expr1) || (usesVarName_list varNames expr2)
usesVarName_list varNames _ = False

usesVarName :: VarName Anno -> Expr Anno -> Bool
usesVarName varnameInp (Var _ _ list) =
  foldl (||) False $ map (\(varname, exprs) -> varname == varnameInp) list
...
replaceAllOccurences_varnamePairs ::
     Fortran Anno -> [VarName Anno] -> [VarName Anno] -> Fortran Anno
replaceAllOccurences_varnamePairs codeSeg originals replacements =
  foldl
    (\accum (v1, v2) -> replaceAllOccurences_varname accum v1 v2)
    codeSeg
    pairs
  where
...
replaceAllOccurences_varname ::
     (Data (a Anno)) => a Anno -> VarName Anno -> VarName Anno -> a Anno
replaceAllOccurences_varname codeSeg original replacement =
  everywhere (mkT (replaceVarname original replacement)) codeSeg

replaceVarname :: VarName Anno -> VarName Anno -> VarName Anno -> VarName Anno
replaceVarname original replacement inp
  | original == inp = replacement
  | otherwise = inp
...
replaceFortran' oldFortran newFortran currentFortran
  | (applyGeneratedSrcSpans oldFortran) ==
      (applyGeneratedSrcSpans currentFortran) =
    normaliseSrcSpan currentFortran newFortran
  | otherwise = currentFortran

replaceProgUnit ast oldProgUnit newProgUnit =
  everywhere (mkT (replaceProgUnit' oldProgUnit newProgUnit)) ast

replaceProgUnit' ::
     ProgUnit Anno -> ProgUnit Anno -> ProgUnit Anno -> ProgUnit Anno
replaceProgUnit' oldProgUnit newProgUnit currentProgUnit
  | (applyGeneratedSrcSpans oldProgUnit) ==
      (applyGeneratedSrcSpans currentProgUnit) =
    normaliseSrcSpan currentProgUnit newProgUnit
...
appendFortran_recursive newFortran (FSeq anno1 src1 (NullStmt _ _) fortran2) =
  FSeq anno1 src1 fortran2 newFortran
appendFortran_recursive newFortran (FSeq anno1 src1 fortran1 (NullStmt _ _)) =
  FSeq anno1 src1 fortran1 newFortran
appendFortran_recursive newFortran (FSeq anno1 src1 fortran1 fortran2) =
  FSeq anno1 src1 fortran1 (appendFortran_recursive newFortran fortran2)
appendFortran_recursive (NullStmt _ _) codeSeg = codeSeg
appendFortran_recursive newFortran (NullStmt _ _) = newFortran
appendFortran_recursive newFortran codeSeg =
  FSeq nullAnno nullSrcSpan codeSeg newFortran
...
removeLoopConstructs_recursive (FSeq anno _ (For _ _ _ _ _ _ fortran1) fortran2) =
  removeLoopConstructs_recursive $ appendFortran_recursive fortran2 fortran1
removeLoopConstructs_recursive (For _ _ _ _ _ _ fortran) =
  removeLoopConstructs_recursive fortran
removeLoopConstructs_recursive (FSeq _ _ fortran (NullStmt _ _)) =
  removeLoopConstructs_recursive fortran
removeLoopConstructs_recursive codeSeg = codeSeg

extractFirstFortran codeSeg =
  head (everything (++) (mkQ [] extractFortran) codeSeg)

extractFirstChildFor ::
     Fortran Anno -> Maybe (Fortran Anno, Fortran Anno, Fortran Anno)
extractFirstChildFor (For _ _ _ _ _ _ fortran)
  | forFound = Just (priorFortran, firstFor, followingFortran)
  | otherwise = Nothing
  where
    allFors = everything (++) (mkQ [] extractForWithFollowing) fortran
    forFound =
      case extractedFor of
        Nothing -> False
        Just a  -> True
    extractedFor = extractForWithFollowing_beta fortran
    (firstFor, followingFortran) =
      fromMaybe (error "extractFirstChildFor") extractedFor
...
extractForWithFollowing (FSeq _ _ fortran1 fortran2) =
  case fortran1 of
...
extractForWithFollowing_beta ::
     Fortran Anno -> Maybe (Fortran Anno, Fortran Anno)
extractForWithFollowing_beta codeSeg =
  case codeSeg of
    FSeq _ _ fortran1 fortran2 ->
      case fortran1 of
  ...
  where
    ...
    recursiveCheck_maybe =
      case recursiveCheck of
        [] -> Nothing
        _  -> head recursiveCheck

extractPriorToFor :: Fortran Anno -> Fortran Anno
extractPriorToFor codeSeg =
  case codeSeg of
...
generateESeq (var:vars) =
  ESeq nullAnno nullSrcSpan (generateESeq vars) (generateVar var)

generateFSeq :: [Fortran Anno] -> Fortran Anno
generateFSeq [] = NullStmt nullAnno nullSrcSpan
generateFSeq (statement:[]) = statement
generateFSeq (statement:statements) =
  FSeq nullAnno nullSrcSpan statement (generateFSeq statements)
...
getEarliestSrcSpan spans =
  Just
    (foldl
       (\accum item ->
          if checkSrcSpanBefore item accum
            then item
            else accum)
       (spans !! 0)
       spans)
...
getEarliestSrcLoc locs =
  Just
    (foldl
       (\accum item ->
          if checkSrcLocBefore item accum
            then item
            else accum)
       (locs !! 0)
       locs)
...
checkSrcLocBefore (SrcLoc file_before line_before column_before) (SrcLoc file_after line_after column_after) =
  (line_before < line_after) ||
  ((line_before == line_after) && (column_before < column_after))

checkSrcSpanAfter :: SrcSpan -> SrcSpan -> Bool
checkSrcSpanAfter ((SrcLoc file_before line_before column_before), _) (_, (SrcLoc file_after line_after column_after)) =
  (line_before > line_after) ||
  ((line_before == line_after) && (column_before > column_after))
...
checkSrcSpanBefore (_, (SrcLoc file_before line_before column_before)) ((SrcLoc file_after line_after column_after), _) =
  (line_before < line_after) ||
  ((line_before == line_after) && (column_before < column_after))
...
checkSrcSpanBefore_line ((SrcLoc file_before line_before column_before), beforeEnd) ((SrcLoc file_after line_after column_after), afterEnd) =
  (line_before < line_after)

checkSrcSpanContainsSrcSpan :: SrcSpan -> SrcSpan -> Bool
checkSrcSpanContainsSrcSpan ((SrcLoc _ outerLS outerCS), (SrcLoc _ outerLE outerCE)) ((SrcLoc _ innerLS innerCS), (SrcLoc _ innerLE innerCE)) =
  outerStartsBefore && outerEndsAfter
  where
    outerStartsBefore =
      (outerLS < innerLS) || (outerLS == innerLS && outerCS < innerCS)
    outerEndsAfter =
      (outerLE > innerLE) || (outerLE == innerLE && outerCE > innerCE)
...
listSubtractWithExemption exempt a b =
  filter (\x -> (elem x exempt) || (notElem x b)) a
...
combineMaps map1 map2 = resultantAnalysis
  where
    ...
    resultantAnalysis =
      foldl
        (\accum (key, value) ->
           DMap.insert key ((DMap.findWithDefault [] key accum) ++ value) accum)
        map1
        map2List

appendToMap :: Ord k => k -> a -> DMap.Map k [a] -> DMap.Map k [a]
appendToMap key item map =
  DMap.insert key ((DMap.findWithDefault [] key map) ++ [item]) map

extractPrimaryReductionOp :: Expr Anno -> Expr Anno -> Maybe (BinOp Anno)
extractPrimaryReductionOp assignee (Bin _ _ op expr1 expr2) =
  case assigneePresent of
    True  -> Just op
    False -> childOp
  where
    ...
    childOp =
      case primaryOp1 of
        ...
        Nothing -> primaryOp2
    assigneePresent =
      applyGeneratedSrcSpans expr1 == applyGeneratedSrcSpans assignee ||
...
extractPrimaryReductionFunction assignee (Var _ _ list) =
  foldl assigneePresent "" standardisedList
  where
    assigneePresent =
      (\accum (var, exprList) ->
         if elem (applyGeneratedSrcSpans assignee) exprList
           then varNameStr var
           else accum)
    standardisedList =
      map
        (\(var, exprList) -> (var, map (applyGeneratedSrcSpans) exprList))
        list
...
evaluateRange ::
     ValueTable -> Expr Anno -> Expr Anno -> Expr Anno -> Maybe ([Float])
              ...
              case stepInt of
                ...
                Just step ->
                  Just
                    (map
                       (fromIntegral)
                       [round start :: Int,(round start :: Int) +
                                           (round step :: Int) .. round end :: Int])
...
evaluateExpr_type vt (Unary _ _ unOp expr) =
  case unOp of
    UMinus _ -> maybeNegative (evaluateExpr_type vt expr)
    Not _    -> Nothing
evaluateExpr_type vt (Var p src lst)
  | varString == "mod" =
    maybeBinOp_integral
      (evaluateExpr_type vt expr1)
      (evaluateExpr_type vt expr2)
      (mod)
...
evaluateExpr_type _ (Con _ _ str)
  | head str == '.' = Just ((read $ tail str) :: Float, Real nullAnno)
  | last str == '.' =
    Just ((read $ take (length str - 1) str) :: Float, Real nullAnno)
...
lookupValueTable str table = value
  where
    (value, typ) =
      case lookupValueTable_type str table of
...
addToValueTable var value table =
  addToValueTable_type var value (Real nullAnno) table -- DMap.insert (varNameStr var) value table

addToValueTable_type ::
     VarName Anno -> Float -> BaseType Anno -> ValueTable -> ValueTable
addToValueTable_type var value typ table =
  DMap.insert (varNameStr var) (value, typ) table
      ...
      case maybeFloat1 of
        ...
        Just (float1, typ1) ->
          case maybeFloat2 of
            ...
            Just (float2, typ2) ->
              Just
                ( fromIntegral (op (round float1) (round float2)) :: Float
                , Integer nullAnno)
...
maybeBinOp ::
  ...
  -> (Float -> Float -> Float)
  -> Maybe (Float, BaseType Anno)
maybeBinOp maybeFloat1 maybeFloat2 op =
  case maybeFloat1 of
    Nothing -> Nothing
    Just (float1, typ1) ->
      case maybeFloat2 of
...
findDeclLine line =
  Data.List.isInfixOf "::" line && head (filter (/= ' ') line) /= '!'
...
findDeclLineVars line =
  if Data.List.isInfixOf "::" line && head (filter (/= ' ') line) /= '!'
    then let line_no_comments = head $ splitDelim "!" line -- (warning line ("<OL:"++line++">"))
...
-- WV
isUseDecl line =
  let chunks = filter (not . null) $ words line
   in if ((not (null chunks)) && (head chunks == "use"))
        then True
        else False -- init (tail (chunks !! 1)) else ""
...
readUsedModuleDecls sourceDir line cppDArgs cppXArgs fixedForm =
  let chunks = filter (not . null) $ words line
      ...
      file_name_root
        | take (length "module_") module_name == "module_" =
          drop (length "module_") module_name
        | otherwise = module_name
                                                                            --        module_name = chunks !! 1
                  -- putStrLn $ "readUsedModuleDecls: " ++ line
   in do test1 <- doesFileExist (sourceDir ++ "/" ++ module_name ++ ".f95")
       ...
       -- FIXME: I am simply ignoring stash and moduleVarTable for now!
         (module_content_str, stash, moduleVarTable) <-
           if test1
             then preProcessingHelper
                    cppDArgs
                    cppXArgs
                    fixedForm
                    True
...
isDeclOnly module_content_str =
  let module_lines = lines module_content_str
      module_lines_no_comments = map (takeWhile (/= '!')) module_lines
      module_lines_no_comments_no_blanks =
        removeBlankLines module_lines_no_comments
      relevant_module_lines =
        filter isRelevantModuleLine module_lines_no_comments_no_blanks
      non_decl_lines = filter (not . findDeclLine) relevant_module_lines
   in null non_decl_lines -- (warning (null non_decl_lines) (show non_decl_lines) )
...
inlineDeclsFromUsedModules ::
  ...
  -> [String]
  -> Bool
  -> IO ([String], ModuleVarsTable)
inlineDeclsFromUsedModules sourceDir False contentLines _ _ _ =
  return (contentLines, DMap.empty)
inlineDeclsFromUsedModules sourceDir True contentLines cppDArgs cppXArgs fixedForm = do
  let hasImplicitNone = length (filter isImplicitNoneDecl contentLines) > 0
  expandedContentLines' <-
    mapM
      (\line ->
         if (isUseDecl line)
           then (readUsedModuleDecls sourceDir line cppDArgs cppXArgs fixedForm)
           else return ([line], ("", [])))
      (filter (not . isImplicitNoneDecl) contentLines)
  let (expandedContentLines, moduleVarTupleList) = unzip expandedContentLines'
          -- now we must reverse this: we want per variable the module
          -- we assume vars are unique, so no conflicts across modules
  let vmtuplst =
        foldl (++) [] $
        map
          (\(mod_name, vars) -> map (\var -> (var, mod_name)) vars)
          moduleVarTupleList -- [ (mod,[vars]) ,...]
      moduleVarTable = DMap.fromList vmtuplst
  let expandedContentLines' = foldl (++) [] expandedContentLines
  let expandedContentLines''
...
addImplicitNone contentLines =
  let (beforeImplicitNoneDecl, afterImplicitNoneDecl, _) =
        foldl
          (\(bls, als, beforeDecl) line ->
             let beforeDecl' =
                   if beforeDecl && (Data.List.isInfixOf "::" line)
                     then False
                     else beforeDecl
              in if beforeDecl'
          ...
          ([], [], True)
          contentLines
   in beforeImplicitNoneDecl ++ ["      implicit none"] ++ afterImplicitNoneDecl

splitDelim :: String -> String -> [String]
splitDelim patt line =
  let (chunks, last_chunk, _) =
        foldl
          (\(chunks, current_chunk, rest_of_line) ch ->
             if Data.List.isPrefixOf patt rest_of_line
               then let rest_of_line' =
                          case Data.List.stripPrefix patt rest_of_line of
                            Just r  -> r
                            Nothing -> rest_of_line
                     in (chunks ++ [current_chunk], "", rest_of_line')
               else if length rest_of_line > 0
                      then let ch':rest_of_line' = rest_of_line
                               current_chunk' = current_chunk ++ [ch']
                            in (chunks, current_chunk', rest_of_line')
                      else (chunks, current_chunk, rest_of_line))
          ([], "", line)
          line
   in chunks ++ [last_chunk]
...
oneVarDeclPerVarDeclLine contentLines =
  let contentLines' :: [[String]]
      contentLines' =
        map
          (\line ->
             if isVarDeclWithMultipleVars line
               then splitOutVarDecls line
               else [line])
          contentLines
   in foldl (++) [] contentLines'

isVarDeclWithMultipleVars :: String -> Bool
isVarDeclWithMultipleVars line =
  let line_no_comments = head $ splitDelim "!" line
      chunks = splitDelim "::" line_no_comments
   in (findDeclLine line_no_comments) &&
      (length chunks == 2) && (Data.List.isInfixOf "," (chunks !! 1))
...
getLoopVars :: Fortran Anno -> [VarName Anno]
getLoopVars (OpenCLMap _ _ vrs vws lvars ilvars stmt1) =
  map (\(v, _, _, _) -> v) lvars
getLoopVars (OpenCLReduce _ _ vrs vws lvars ilvars rvarexprs stmt1) =
  map (\(v, _, _, _) -> v) lvars
getLoopVars (OpenCLStencil _ _ _ body) = getLoopVars body
getLoopVars (FSeq _ _ f1 f2) = getLoopVars f1 ++ getLoopVars f2
getLoopVars _ = []
