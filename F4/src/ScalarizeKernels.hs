...
deleteArrayArgs streams kernel = Sub subAnno
                                     subSrcSpan
                                     returnType
                                     name
 ...
 where
  ...
  arrayArgsRemoved = filter
    (\(ArgName _ name) -> name `Set.notMember` replacedByStream)
    allArgs
  newArgsNode = buildAstSeq (ASeq nullAnno) (NullArg nullAnno) arrayArgsRemoved
...
replaceArrayDeclarations replacementMap kernel = trace
  ("map size = " ++ show (DMap.size replacementMap))
  newProgUnit
 where
...
matchStreamingVarsToArrayDecls streams arrays = matched
 where
  arrayNameToStreamMap =
    DMap.fromList $ map (\grp -> ((fst . head) grp, map snd grp)) groupedByArray
  groupedByArray = groupBy (\(n1, _) (n2, _) -> n1 == n2)
    $ map (\s@(Stream _ arrayName _ _) -> (arrayName, s)) streams
  arrayNameToArrayMap = DMap.fromList
    $ map (\arr -> ((getNameFromVarName . varName) arr, arr)) arrays
  matched = DMap.intersectionWith (\arr stream -> (arr, stream))
                                  arrayNameToArrayMap
                                  arrayNameToStreamMap

replaceArrayDeclWithStreamingVarDecls
  :: DMap.Map String (Array, [Stream Anno]) -> Decl Anno -> [Decl Anno]
replaceArrayDeclWithStreamingVarDecls replacementMap currentDecl = map
  (\(Stream name _ _ _) -> makeDecl name)
  streams
 where
  ...
  attrsWithDimensionsRemove = filter
    (\case
      (Dimension _ _) -> False
      _               -> True
    )
...
subsitutionStreamVars arrays streams expr
  | isArrayAccess arrays expr = makeVar
