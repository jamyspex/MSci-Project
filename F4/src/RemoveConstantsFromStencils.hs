...
processLoopNest (loopNest, arrayAccesses) = do
  ...
  when (isJust highestDimArrayAccessedWithConstant)
  ...
  return loopNest
 where
  loopVarPosTuples = getLoopVarConstantPosTuples accessesValidated
                                                 nestingDirection
                                                 withSyntheticLoopsInserted
  withSyntheticLoopsInserted = insertSyntheticLoops
...
getLoopVarConstantPosTuples arrayAccesses nestDirection loopNest = concatMap
  (buildTuple loopVarsOrderedByNesting)
  arrayAccesses
 where
  loopVarsOrderedByNesting = case nestDirection of
  ...
  mostIndicesCount = trace
    ("loopVarsOrderedByNesting = " ++ show loopVarsOrderedByNesting)
    (length . indices . getHighestDimensionArrayAccessNoMaybe)
    arrayAccesses
...
addLoops dims insertLoop wholeLoopNest = addLoops' maxNestLevel
                                                   0
                                                   dims
                                                   insertLoop
                                                   wholeLoopNest
  where maxNestLevel = length dims - 1
...
addLoops' maxLevel level dims insertLoop loopNest
  ...
  | level == maxLevel && loopBodyStatementsOnly loopNest
  = loopNest
 ...
 where
  ...
  originalFor = case getNearestFor loopNest of
...
getHighestDimensionArrayAccessNoMaybe = maximumBy
  (\a1 a2 ->
...
getHighestDimensionArrayAccess allArrayAccesses = if null onlyWithConstants
  then Nothing
  else Just mostConstants
 where
  mostConstants = maximumBy
    (\a1 a2 ->
      length (declaredDimensions a1) `compare` length (declaredDimensions a2)
    )
    onlyWithConstants
  onlyWithConstants = filter
    (\AA {..} -> any
      (\case
        LoopVar _ -> False
        Const   _ -> True
...
checkOne loopVars arrayAccess = case (forward, backward) of
  (True , True ) -> Either
  (True , _    ) -> Normal
  (_    , True ) -> Reverse
 ...
 where
  ...
  go misMatchCount _ _ | misMatchCount == 2 = False
                       | otherwise          = True
...
parseArrayAccesses allArrays loopNest = uniqueArrayAccesses
 where
  ...
  allArrayAccessExprs = getAllArrayAccessesWithMatchingArray allArrays loopNest
  buildArrayAccess :: (Expr Anno, Array) -> ArrayAccess
  buildArrayAccess (accessExpr, Array {..}) = AA
    { arrayName          = name
...
loopBodyStatementsOnly fortran = case fortran of
  For{}                         -> False
...
loopBodyOnlyContainsLoop fortran = case fortran of
  For{}                     -> True
