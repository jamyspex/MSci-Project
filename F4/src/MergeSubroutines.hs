...
mergeSubsToBeParallelised srt =
  DMap.insert (subName mergedSubRec) mergedSubRec originalsRemoved
  where
...
mergeSubs argTransToSubRecs = (uniqueArgTrans, mergedSubRec)
  where
    ...
    uniqueDecls =
      sortBy getDeclOrdering $
      getUniqueDecls $ getAllDecls subsWithParamsReplaced
    uniqueArgs = getUniqueArgs $ getAllArgs subsWithParamsReplaced
    uniqueArgTrans =
      getUniqueArgTrans $
      concatMap (\(argTrans, _) -> argTrans) argTransToSubRecs
...
getArgTransSubroutinePairs srt =
  map (\(_, argTrans, subrec) -> (argTrans, subrec)) sorted
  where
    sorted = sortBy (getCallOrdering `on` fst3) allPairs
    allPairs =
      concatMap
        (\caller -> concatMap (getPair caller) $ getCalled caller)
        callers
    getCalled callerSubRec = DMap.keys $ argTranslations callerSubRec
    getPair ::
         SubRec -> String -> [(Fortran Anno, [ArgumentTranslation], SubRec)]
...
findConflicts localDecls argTrans =
  filter (\decl -> decl `elem` argNames) localDecls
  where
...
orderDeclsByIntents (Decl _ _ _ typeDecl1) (Decl _ _ _ typeDecl2) =
  ...
  where
    ...
    getIntent attrs =
      case ((concatMap (getIntentAttrs)) . getAttrs) attrs of
...
renameParameter argTrans subRec =
  subRec {subAst = everywhere (mkT renameQuery) $ subAst subRec}
  where
...
replaceOneParamUsageWithArg argTrans subRec =
  subRec {subAst = everywhere (mkT replaceQuery) $ subAst subRec}
  where
