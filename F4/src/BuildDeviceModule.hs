...
buildDeviceModule
  :: Int -> [PipelineStage] -> IO (String, ProgUnit Anno, [KernelCallingData])
buildDeviceModule numPipelines pipeline = do
 ...
 where
  ...
  allPipes = concatMap
    (\(k, sm, ma) ->
      writtenPipes k ++ concatMap writtenPipes sm ++ concatMap writtenPipes ma
    )
    pipeline
  ...
  nameSplitIdx = if numPipelines > 1 then 1 else 0
  moduleName =
    LU.join
        "_"
...
generateStage (kernel, smartCache, memAccess) =
 ...
 where
  ...
  (smartCacheKernel, smartCacheCallingData) = maybe
    ([], [])
    (\s -> let (sm, kcd) = generateSmartCache s in ([sm], [kcd]))
    smartCache
  (memoryReaderKernels, memReadKCD) = unzip $ map generateMemoryReader $ filter
    (\case
      MemoryReader{} -> True
      _              -> False
    )
  ...
  (memoryWriterKernels, memWriteKCD) =
    unzip $ map generateMemoryWriter $ filter
      (\case
        MemoryWriter{} -> True
        _              -> False
      )
...
generatePipeInitCall (Pipe _ _ pipeName streamValueType _) = call
  (getCallName streamValueType)
  [pipeName]
 where
  getCallName streamValueType = case streamValueType of
    Float -> "ocl_pipe_real"
    _     -> "ocl_pipe_int"
