...
import           LoopAnalysis                   ( analyseLoop_map
                                                , analyseLoop_reduce
                                                , getErrorAnnotations
                                                , getReads
...
paralleliseProgUnit_foldl
  ...
  -> String
  -> (SubroutineTable, [(String, String)])
paralleliseProgUnit_foldl ioWriteSubroutines originalTable (accumSubTable, annoListing) subName
  = (newSubTable, annoListing ++ [(filename, parAnno)])
 where
  subrec = DMap.findWithDefault (error "paralleliseProgUnit_foldl")
                                subName
                                originalTable
  ...
  parallelisedProgUnit    = everywhere
    (mkT (paralleliseBlock filename originalTable accessAnalysis))
    progUnitfoldedConstants
  ...
  newSubTable = DMap.insert
    subName
    (MkSubRec parallelisedProgUnit
              filename
              []
              subName
              (argTranslations subrec)
              (parallelise subrec)
    )
    accumSubTable
...
combineKernelProgUnit_foldl
  ...
  -> String
  -> (SubroutineTable, [(String, String)])
combineKernelProgUnit_foldl loopFusionBound (subTable, annoListing) subName =
  (newSubTable, annoListing ++ [(filename, combAnno)])
 where
        -- get the record from the subroutine table
  subrec =
    DMap.findWithDefault (error "paralleliseProgUnit_foldl") subName subTable
  ...
  combinedProgUnit =
    combineKernelsProgUnit loopFusionBound (removeAllAnnotations progUnit)
  combAnno    = compileAnnotationListing combinedProgUnit
  newSubTable = DMap.insert
    subName
    (MkSubRec combinedProgUnit
              filename
              []
              subName
              (argTranslations subrec)
...
paralleliseBlock
  :: String -> SubroutineTable -> VarAccessAnalysis -> Block Anno -> Block Anno
paralleliseBlock filename subTable accessAnalysis block = gmapT
  (mkT (isolateAndParalleliseForLoops filename subTable accessAnalysis))
  block
...
isolateAndParalleliseForLoops
  ...
  -> Fortran Anno
  -> Fortran Anno
isolateAndParalleliseForLoops filename subTable accessAnalysis inp =
  case inp of
    For _ _ _ _ _ _ _ -> paralleliseLoop filename
                                         []
                                         accessAnalysis
                                         subTable
                                         recusivelyAnalysedNode
     where
      recusivelyAnalysedNode = gmapT
        (mkT (isolateAndParalleliseForLoops filename subTable accessAnalysis))
        inp
    _ -> gmapT
      (mkT (isolateAndParalleliseForLoops filename subTable accessAnalysis))
...
paralleliseLoop filename loopVars accessAnalysis subTable loop = transformedAst
 where
  ...
  --
  mapAttempt     = paralleliseLoop_map filename
                                       loop
                                       newLoopVars
                                       nonTempVars
                                       prexistingVars
  ...
  reduceAttempt   = paralleliseLoop_reduce filename
                                           mapAttempt_ast
                                           newLoopVars
                                           nonTempVars
                                           prexistingVars
  ...
  reduceWithOuterIterationAttempt = paralleliseLoop_reduceWithOuterIteration
    filename
    reduceAttempt_ast
    Nothing
    newLoopVars
...
extractWrites (Assg _ _ (Var _ _ list) _) =
  map (\(varname, exprs) -> varname) list
extractWrites _ = []
...
paralleliseLoop_map filename loop loopVarNames nonTempVars prexistingVars dependencies accessAnalysis subTable
  ...
  = ( True
    , appendAnnotation
      mapCode
      (compilerName ++ ": Map at " ++ errorLocationFormatting (srcSpan loop))
      ""
    )
  | otherwise
...
-- WV: def:                                            analyseLoop_map  comment loopVars loopWrites nonTempVars prexistingVars accessAnalysis dependencies subTable codeSeg
  loopAnalysis = analyseLoop_map "Cannot map: "
                                 []
                                 loopWrites
                                 nonTempVars
                                 prexistingVars
  ...
  (loopCarriedDeps_bool, evaluated_bool, loopCarriedDeps) =
    loopCarriedDependencyCheck loop

  errors_map' = case loopCarriedDeps_bool of
    True -> case evaluated_bool of
      True -> DMap.insert
        (outputTab ++ "Cannot map: Loop carried dependency detected:\n")
        (formatLoopCarriedDependencies loopCarriedDeps)
        errors_map
      False -> DMap.insert
        (  outputTab
        ++ "Cannot map: Loop carried dependency possible (not evaluated):\n"
        )
        (formatLoopCarriedDependencies loopCarriedDeps)
        errors_map
    False -> errors_map
  ...
  startVarNames =
    foldl (\accum (_, x, _, _) -> accum ++ extractVarNames x) [] loopVariables
  endVarNames =
    foldl (\accum (_, _, x, _) -> accum ++ extractVarNames x) [] loopVariables
  stepVarNames =
    foldl (\accum (_, _, _, x) -> accum ++ extractVarNames x) [] loopVariables

  varNames_loopConditions = listSubtract
    (listRemoveDuplications (startVarNames ++ endVarNames ++ stepVarNames))
    loopVarNames
  containedLoopIteratorVarNames =
    (map (\(a, _, _, _) -> a) (loopCondtions_query loop))

  reads_map_varnames = foldl (++) [] (map extractVarNames reads_map)
  readArgs           = listRemoveDuplications
    $ listSubtract reads_map_varnames containedLoopIteratorVarNames    -- List of arguments to kernel that are READ
  ...
  writtenArgs         = listRemoveDuplications
    $ listSubtract writes_map_varnames containedLoopIteratorVarNames     -- List of arguments to kernel that are WRITTEN
  -- WV20170426
  iterLoopVariables = []

  mapCode           = OpenCLMap nullAnno
                                (generateSrcSpan filename (srcSpan loop))     -- Node to represent the data needed for an OpenCL map kernel -- WV20170426
...
paralleliseLoop_reduce filename loop loopVarNames nonTempVars prexistingVars dependencies accessAnalysis
  ...
  = ( True
    , appendAnnotation
      reductionCode'
      (compilerName ++ ": Reduction at " ++ errorLocationFormatting
        (srcSpan loop)
      )
      ""
    )
  | otherwise
  = (False, appendAnnotationMap loop errors_reduce')
 where
  loopWrites   = extractWrites_query loop
  loopAnalysis = analyseLoop_reduce "Cannot reduce: "
                                    []
                                    []
                                    loopWrites
                                    nonTempVars
  ...
  errors_reduce' = case loopCarriedDeps_bool of
    True -> case evaluated_bool of
      True -> DMap.insert
        (outputTab ++ "Cannot reduce: Loop carried dependency detected:\n")
        (formatLoopCarriedDependencies loopCarriedDeps)
        errors_reduce
      False -> DMap.insert
        (  outputTab
        ++ "Cannot reduce: Loop carried dependency possible (not evaluated):\n"
        )
        (formatLoopCarriedDependencies loopCarriedDeps)
        errors_reduce
    False -> errors_reduce
  ...
  readArgs =
    ...
    $ listSubtract reads_map_varnames (varNames_loopConditions)
    )    -- List of arguments to kernel that are READ
  -- readArgs = (listRemoveDuplications $ listSubtract reads_map_varnames (containedLoopIteratorVarNames ++ varNames_loopConditions)    )    -- List of arguments to kernel that are READ

  writes_map_varnames = foldl (++) [] (map extractVarNames writes_reduce)
  writtenArgs =
    ( listRemoveDuplications
    $ listSubtract writes_map_varnames containedLoopIteratorVarNames
    )     -- List of arguments to kernel that are WRITTEN

  allReductionVarNames =
    (foldl (\accum item -> accum ++ extractVarNames item) [] reductionVarNames)
  reductioNVarInfo =
    (listRemoveDuplications
      (foldl
        (\accum item ->
          accum
            ++ [(item, getValueAtSrcSpan item (srcSpan loop) accessAnalysis)]
        )
        []
        allReductionVarNames
      )
  ...
  reductionCode     = OpenCLReduce nullAnno
                                   (generateSrcSpan filename (srcSpan loop))   -- WV20170426
                                   readArgs            -- List of arguments to kernel that are READ
                                   writtenArgs         -- List of arguments to kernel that are WRITTEN
                                   loopVariables        -- Loop variables of nested maps
                                   iterLoopVariables -- WV20170426
...
updateIterLoopVars loopVarNames (OpenCLMap f1 f2 f3 f4 f5 ilvars stmt) =
  OpenCLMap f1 f2 f3 f4 f5 (ilvars ++ loopVarNames) stmt
updateIterLoopVars loopVarNames (OpenCLReduce f1 f2 f3 f4 f5 ilvars f6 stmt) =
  OpenCLReduce f1 f2 f3 f4 f5 (ilvars ++ loopVarNames) f6 stmt
...
paralleliseLoop_reduceWithOuterIteration filename loop Nothing loopVarNames loopVarNames' nonTempVars prexistingVars dependencies accessAnalysis
  ...
  = (False, loop')
  | otherwise
  = (reduceWithOuterIterationAttempt_bool, newAst')
 where
  ...
  (priorFortran, nextFor, followingFortran) = case nextFor_maybe of
    Nothing -> error
      "paralleliseLoop_reduceWithOuterIteration: nextFor_maybe is Nothing"
      25
    Just a -> a
  newLoopVarNames = case getLoopVar nextFor of
    Just a  -> loopVarNames' ++ [a]
    Nothing -> loopVarNames'
  (reduceWithOuterIterationAttempt_bool, reduceWithOuterIterationAttempt_ast) =
    paralleliseLoop_reduceWithOuterIteration filename
                                             loop
                                             (Just nextFor)
                                             loopVarNames
                                             newLoopVarNames
  ...
  newAst = case reduceWithOuterIterationAttempt_ast of
    For a1 a2 a3 a4 a5 a6 fortran -> For
      a1
      a2
      a3
      a4
      ...
      (appendFortran_recursive
        (appendFortran_recursive followingFortran fortran)
        priorFortran
      )
...
paralleliseLoop_reduceWithOuterIteration filename iteratingLoop (Just parallelLoop) loopVarNames loopVarNames' nonTempVars prexistingVars dependencies accessAnalysis
  ...
  = ( True
    , appendAnnotation
      reduceWithOuterIterationCode
      (  compilerName
      ++ ": Reduction with outer iteration at "
      ++ errorLocationFormatting (srcSpan iteratingLoop)
      ++ " with parallel loop at "
      ++ errorLocationFormatting (srcSpan parallelLoop')
      )
      ""
    )
  | nextFor_maybe /= Nothing
  = paralleliseLoop_reduceWithOuterIteration
    filename
    (appendAnnotationMap iteratingLoop errors_reduce')
    (Just nextFor)
    loopVarNames
    newLoopVarNames
    nonTempVars
    ...
    accessAnalysis
  | otherwise
  = (False, appendAnnotationMap iteratingLoop errors_reduce')
 where
  ...
  loopAnalysis  = analyseLoop_reduce reduceWithOuterIterationComment
                                     []
                                     []
                                     loopWrites
                                     nonTempVars
  ...
  -- WV: CHECK THIS!
  iteratingLoop' =
    everywhere (mkT (updateIterLoopVars loopVarNames)) iteratingLoop -- warning iteratingLoop ("! ITERATIVE LOOP (2): \n"++(miniPPF iteratingLoop)++"\n! END ITERATIVE LOOP (2)\n" )
  iteratingLoopVars = listSubtract (extractLoopVars iteratingLoop')
                                   (extractLoopVars parallelLoop')

  (loopCarriedDeps_bool, evaluated_bool, loopCarriedDeps) =
    loopCarriedDependencyCheck_reductionWithIteration iteratingLoop
                                                      parallelLoop'
  errors_reduce' = case loopCarriedDeps_bool of
    True -> case evaluated_bool of
      True -> DMap.insert
  ...
  (priorFortran, nextFor, followingFortran) = case nextFor_maybe of
    Nothing ->
      error "paralleliseLoop_reduceWithOuterIteration: nextFor is Nothing"
    Just a -> a
  newLoopVarNames = case getLoopVar nextFor of
    Just a  -> loopVarNames' ++ [a]
    Nothing -> loopVarNames'
  newAst = appendFortran_recursive
    followingFortran
    (appendFortran_recursive reductionCode priorFortran)
  ...
  reduceWithOuterIterationCode = case iteratingLoop of
    ...
    _                             -> error
      "paralleliseLoop_reduceWithOuterIteration: iterating loop is not FOR"

  varNames_loopConditions = listSubtract
    (listRemoveDuplications (startVarNames ++ endVarNames ++ stepVarNames))
    loopVarNames
  containedLoopIteratorVarNames =
    (map (\(a, _, _, _) -> a) (loopCondtions_query parallelLoop'))

  reads_map_varnames = foldl (++) [] (map extractVarNames reads_reduce)
  readArgs =
    (listRemoveDuplications $ listSubtract
      reads_map_varnames
      (containedLoopIteratorVarNames ++ iteratingLoopVars)
    )    -- List of arguments to kernel that are READ
    ...
    = (listRemoveDuplications
        (foldl
          (\accum item ->
            accum
              ++ [ ( item
                   , getValueAtSrcSpan item
                                       (srcSpan parallelLoop')
                                       accessAnalysis
                   )
                 ]
          )
  ...
  reductionCode     = OpenCLReduce
    nullAnno
    (generateSrcSpan filename (srcSpan parallelLoop'))  -- WV20170426
    readArgs            -- List of arguments to kernel that are READ
    writtenArgs         -- List of arguments to kernel that are WRITTEN
    loopVariables        -- Loop variables of nested maps
    iterLoopVariables -- WV20170426
  ...
  reduceWithOuterIterationComment =
    "Cannot iterative reduce (iter:"
      ++ (errorLocationFormatting $ srcSpan iteratingLoop)
      ++ ", par:"
      ++ (errorLocationFormatting $ srcSpan parallelLoop')
      ++ "): "
...
loopCondtions_query
  :: (Typeable p, Data p) => Fortran p -> [(VarName p, Expr p, Expr p, Expr p)]
...
compileAnnotationListing codeSeg =
  everything (++) (mkQ [] getAnnotations) codeSeg
...
getAnnotations codeSeg = case (tag codeSeg) == nullAnno of
  ...
  False -> leadMessageCheck
   where
    leadMessageCheck =
      case DMap.findWithDefault [] (head keys) errorMap == [""] of
        True  -> errorListing
        False -> leadMessage ++ errorListing ++ "\n"
    leadMessage =
      compilerName
        ++ ": Cannot parallelise loop at "
        ++ errorLocationFormatting (srcSpan codeSeg)
        ++ "\n"
    errorListing = foldl
      (\errorDescription key ->
        errorDescription
          ++ key
          ++ (foldl
               (\errorInstance item -> errorInstance ++ "\t" ++ item)
               ""
               (applyAnnotationFormatting
                 2
                 (listRemoveDuplications (DMap.findWithDefault [] key errorMap))
               )
             )
          ++ "\n"
      )
...
applyAnnotationFormatting itemsPerLine items = formattedList
 where
  ...
  formattedList  = map
    (\(index, item) ->
      if (mod index itemsPerLine) == 0 && index /= (length items)
        then item ++ "\n"
        else item
    )
    indexoredItems
...
formatLoopCarriedDependencies ((readExpr, writtenExpr) : exprs) =
  [ outputTab
    ++ (outputExprFormatting writtenExpr)
    ++ " -> "
    ++ (outputExprFormatting readExpr)
    ]
    ++ formatLoopCarriedDependencies exprs
...
exprListContainsVarNames
  :: (Typeable p, Data p, Eq p) => [Expr p] -> [VarName p] -> Bool
exprListContainsVarNames contains container =
  all (== True) (everything (++) (mkQ [] (varNameCheck container)) contains)
