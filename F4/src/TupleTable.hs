...
tupleTableElementGreaterThan (x : xs) (y : ys)
  | x > y  = True
  | x == y = tupleTableElementGreaterThan xs ys
  | x < y  = False

getMostTuple :: TupleTable -> [Int]
getMostTuple (LoopIterRecord table) =
  [leastValue] ++ (getMostTuple (DMap.findWithDefault Empty leastValue table))
 where
...
getLeastTuple (LoopIterRecord table) =
  [leastValue] ++ (getLeastTuple (DMap.findWithDefault Empty leastValue table))
 where
  allowedValues = DMap.keys table
...
insertIntoTupleTable (indices) (LoopIterRecord table) =
  case DMap.lookup (head indices) table of
    Just subTable -> LoopIterRecord
      (DMap.insert (head indices)
                   (insertIntoTupleTable (tail indices) subTable)
                   table
      )
    Nothing -> LoopIterRecord
      (DMap.insert (head indices) (createTupleTable (tail indices)) table)

collapseIterTable :: TupleTable -> TupleTable
collapseIterTable (LoopIterRecord iterTable) = foldl
  (\accum (key, newTable) -> insertIfNotRepresented key newTable accum)
  Empty
  (zip allowedValues subTables)
 where
...
insertIfNotRepresented key newItem Empty =
  LoopIterRecord (DMap.insert key newItem DMap.empty)
insertIfNotRepresented key newItem (LoopIterRecord table) =
  if not (elem newItem representedItems)
    then LoopIterRecord (DMap.insert key newItem table)
    else LoopIterRecord table
 where
  representedItems =
    map (\x -> DMap.findWithDefault Empty x table) (DMap.keys table)

createTupleTable :: [Int] -> TupleTable
createTupleTable []      = Empty
createTupleTable indices = LoopIterRecord
  (DMap.insert (head indices) (createTupleTable (tail indices)) DMap.empty)

lookupTupleTable :: [Int] -> TupleTable -> Maybe (TupleTable)
lookupTupleTable [index] (LoopIterRecord table) = DMap.lookup index table
lookupTupleTable (index : indices) (LoopIterRecord table) =
  lookupTupleTable indices (DMap.findWithDefault Empty index table)
