...
arrayToStream loopVars array = Stream
  (buildStreamName array loopVars)
  (getArrayName array)
  (getStreamValueTypeFromArrayBaseType array)
  (dims array)

getStreamValueTypeFromArrayBaseType array = case bt of
  Real    _ -> Float
  Integer _ -> Int
  _         -> error ("Cannot create stream of base type: " ++ show bt)
  where bt = arrayValueType array
