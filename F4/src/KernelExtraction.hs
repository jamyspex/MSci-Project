...
-- FIXME passing in the loop vars here and just appending seems like a big hack
-- but it works.
--
-- On reflection I think all that needs to make it more robust is
-- to ensure we are appending the loop vars in correct order.
...
-- FIXME this is the hacky bit that assumes
--      1) The loopVars in the OpenCLMap/OpenCLReduce
--      are ordered correctly.
--      2) The order the variables are used to index
--      arrays is the same across all instances.
buildStreamName array loopVars =
  getArrayName array ++ concatMap ("_" ++) loopVars

-- FIXME Same thing here for stencil streams
getInputStreams
...
getInputStreams requiredStencils requiredArrays loopVars =
 ...
 where
  ...
  getStencilStreams (array, stencils) = map curriedCon stencils
   where
    curriedCon = StencilStream (buildStreamName array loopVars)
                               (getArrayName array)
                               Float
                               (dims array)
...
matchArraysToStencils arrays stencils =
 ...
 where
  ...
  arrayMap       = DMap.fromList
    $ map (\a -> ((getNameFromVarName . arrayVarName) a, a)) arrays
