...
truncateAtLength lineLength file =
  unlines $ concatMap (truncateLine False lineLength) fileLines
  where
    ...
    truncateLine previousTrunc lineLength line
      ...
      | previousTrunc =
        ("&" ++ take lineLength line ++ "&") :
        truncateLine True lineLength (drop lineLength line)
      | head (dropWhile isSpace line) == '!' = [line] -- don't truncate comments
      | length line <= lineLength = [line]
      | not $ all isSpace (drop lineLength line) =
        (take lineLength line ++ "&") :
        truncateLine True lineLength (drop lineLength line)
      | otherwise = [line]
...
showArg (Arg _ argname _) =
  let arg_str = showArgName argname
   in if arg_str == ""
        then ""
        else "(" ++ arg_str ++ ")"
...
miniPPP (Main _ _ subname args block ps) =
  "! Generated code\n" ++
  "program " ++
  showSubName subname ++
  " " ++
...
showAttr attr =
  let attr_show = map toLower (show attr)
      attr_str = head $ words attr_show
   in attr_str

showType bt e1 e2 =
  let bt_show = map toLower (show bt)
...
miniPPD decl =
  case decl
    ...
    Decl _ _ ttups ty ->
      let ty_str =
            case ty of
              BaseType _ bt attrs e1 e2 ->
                intercalate ", " (showType bt e1 e2 : map miniPPAttr attrs)
              ArrayT _ expr_tups bt attrs e1 e2 ->
                intercalate ", " (showType bt e1 e2 : map miniPPAttr attrs) ++
                "<<" ++ show expr_tups ++ ">>"
          ttups_str =
            intercalate
...
miniPPProgUnit prog =
  ...
  case prog of
    ...
    Sub _ _ _ (SubName _ subname) args b ->
      "subroutine " ++
      subname ++
      showArg args ++
...
miniPPFT stmt tab =
  case stmt of
    ...
    TextStmt _ _ text -> tab ++ text
    OpenCLMap _ _ vrs vws lvars ilvars stmt1 ->
      "! OpenCLMap ( " ++
      showVarLst vrs ++
      "," ++
      showVarLst vws ++
    ...
    Write _ _ specs exprs ->
      tab ++
      "write(" ++
      miniPPSpecs specs tab ++
      ")(" ++ intercalate ", " (map miniPP exprs) ++ ")"
...
showStencils tab [Stencil _ dimen points coords (VarName _ name)] =
  ...
  show points ++
  " point stencil on " ++
  show dimen ++ "D array " ++ name ++ ": " ++ show coords
...
showLoopVar (VarName _ v, e1, e2, e3) =
  "(" ++ v ++ "," ++ miniPP e1 ++ "," ++ miniPP e2 ++ "," ++ miniPP e3 ++ ")"
...
miniPP expr =
  case expr of
    ...
    (Var _ _ [(VarName _ v, es)]) ->
      v ++
      (if null es
         then ""
         else "(" ++ intercalate "," (map miniPP es) ++ ")")
    ...
    _ -> "! UNSUPPORTED in miniPP ! " ++ show expr
