...
calculateSmartCacheDetailsForStream kernel itOrder sten
  ...
  where
    ...
    updatedCoords =
      if centralPointNeedsAdded
        then centralIdx : coords
        else coords
    toProcess =
      StencilStream
        name
  ...
  where
    ...
    combineReaches ((idx1, idx2), firstIter, areaSoFar) component =
      if firstIter || i1 < 0 || i2 > 0
        then ( (idx1, idx2)
             , False
             , areaSoFar + calculateReach component (idx1, idx2))
...
compareIndices i1 i2 iterationOrder =
  if sameLength
    then orderExpr
...
test stream@(StencilStream _ _ _ _ stencil) numBlocksShouldBe startShouldBeIdx endShouldBeIdx maxPosOffsetShouldBe maxNegOffsetShouldBe =
  trace
    ("num blocks: " ++
     ...
     "\n" ++
     "max pos offset: " ++
     show maxPosOffsetShouldBe ++ " == " ++ show maxPosOffset ++ "\n") $
  numBlocksShouldBe == requiredBufferSize &&
  startShouldBe == startIndex &&
  endShouldBe == endIndex &&
  length stencilIndices == (length startToPointDistances + 1) &&
  maxNegOffsetShouldBe == maxNegOffset && maxPosOffsetShouldBe == maxPosOffset
  where
...
-- Test data
nonSymetricalLargerThan1Offset =
  StencilStream
    "test"
    "test"
    Float
...
crossTestData3DZeroBasedIndex_8x8x8 =
  StencilStream
    "test"
    "test"
    Float
...
extremesCrossTestData3D_10x6x8 =
  StencilStream
    "test"
    "test"
    Float
...
crossWithExtraPointsToBeIgnoredTestData3D_10x6x8 =
  StencilStream
    "test"
    "test"
    Float
...
crossWithExtraPointsNotIgnoredTestData3D_10x6x8 =
  StencilStream
    "test"
    "test"
    Float
...
extremitiesOfCrossNotIgnore_10x6x8 =
  StencilStream
    "test"
    "test"
    Float
...
nonSymetricTestDataExtremities_10x6x8 =
  StencilStream
    "test"
    "test"
    Float
