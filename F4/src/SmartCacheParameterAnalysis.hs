...
calculateSmartCacheDetailsForStream kernel itOrder sten = details
 where
  details = SmartCacheDetailsForStream
  ...
  (maxNumBlocks, (maxStart, maxEnd)) =
    ...
    --     getStreamName sten ++ "\nkernel = \n" ++ show (fromJust kernel))
          . sortStencils) centralIdxRemoved --all
...
calculateSmartCacheSizeForAllPairsOfStencilPoints iterationOrder (StencilStream streamName _ _ arrayDimens stencil)
  = trace ("iterationOrder = " ++ show iterationOrder) stencilSizesAndIndexPairs
    -- trace
 ...
 where
  ...
  stencilSizesAndIndexPairs = map
    ...
    (trace
      ...
      ++ "\n length smallIndexFirstOnly = "
      ++ show (length smallIndexFirstOnly)
      )
      smallIndexFirstOnly
    )
  go (l1, l2) =
