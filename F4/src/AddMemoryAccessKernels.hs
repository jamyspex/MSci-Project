    ...
    = if streamAvailable
      then []
        ...
        [ MemoryReader
            ...
            , writtenPipes       = []
            , sharedData         = NullPipeLineData
            , stageNumber        = currentStageIdx
            }
        ]
    where streamAvailable = Set.member arrayName availableStreams
...
addMemoryWriter pipeline = init pipeline ++ [updatedFinalStage]
 where
  ...
  memoryWriter = buildMemoryWriter (name lastKernel)
                                   (stageNumber lastKernel)
                                   (outputStreams lastKernel)
  updatedFinalStage = (lastKernel, smartcache, memoryWriter : memoryReaders)

buildMemoryWriter
  :: String -> Int -> [Stream Anno] -> PipelineItem SharedPipelineData
buildMemoryWriter kernelName stageNum streams = MemoryWriter
