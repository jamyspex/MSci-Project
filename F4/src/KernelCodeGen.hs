...
generateKernels pipeline = do
  ...
  return kernels
  where kernels = map (\(k, _, _) -> generateKernelCode k) pipeline
...
generateKernelCode mapKern@Map {..} = (kernel, callingData)
 where
  ...
  callingData = KCD
    { argPositions = imap (\idx (ArgName _ name) -> (idx, name))
                          loopVarArgsRemoved
    }
  loopVarName = "index"
...
getLoopVarNames kernelBody = map getNameFromVarName loopVars
  where loopVars = everything (++) (mkQ [] getLoopVars) kernelBody
...
stripOpenCLAnnos kernelBody = annosRemoved
 where
  ...
  removeAnnosQuery fortran = case fortran of
...
generatePipeReadsMapKernel Map {..} =
 ...
 where
  ...
  outputStreamsMap = DMap.fromList $ map
