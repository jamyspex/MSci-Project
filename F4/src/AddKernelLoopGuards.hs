...
addLoopGuards subRec = subRec
  { subAst = everywhere (mkT (addLoopGuardToMapOrReduce declRangeMap arrays))
                        ast
  }
 where
  ast          = subAst subRec
...
addLoopGuardToMapOrReduce arrayDeclDimMap arrays fortran@(OpenCLMap _ _ _ _ _ _ _)
  = let
      (OpenCLMap oclAnno oclSrcSpan readVars writtenVars loopVars enclosedLoopVars body)
        = fortran
      newBody = addLoopGuardToMapOrReduce' arrayDeclDimMap
                                           arrays
                                           fortran
                                           loopVars
                                           body
...
addLoopGuardToMapOrReduce arrayDeclDimMap arrays fortran@(OpenCLReduce _ _ _ _ _ _ _ _)
  = let
      (OpenCLReduce oclAnno oclSrcSpan readVars writtenVars loopVars enclosedLoopVars reductionVars body)
        = fortran
      newBody = addLoopGuardToMapOrReduce' arrayDeclDimMap
                                           arrays
                                           fortran
...
addLoopGuardToMapOrReduce' arrayDeclDimMap arrays fortran loopVars body =
 ...
 where
  ...
  indexPositions   = validateIndexingAndMakeUnique
  ...
  singleConditions = map
    (\(loopVarName, bound) -> buildLoopGuard loopVarName bound)
    duplicatesRemoved
...
validateIndexingAndMakeUnique positions = if valid
  then unique
...
groupByArrayAndIndex input = groupBy
  (\(an1, in1, _) (an2, in2, _) -> an1 == an2 && in1 == in2)
  sortedByArrayName
 where
  sortedByArrayName = sortBy
    (\(an1, in1, _) (an2, in2, _) -> (an1 `compare` an2) <> (in1 `compare` in2))
    input
...
formatMapEntries ((_, _), cmi) = (loopVarName, accessLWB, accessUPB)
  where (loopVarName, (accessLWB, accessUPB)) = accessItem cmi
...
getRequiredGuards combinedMapItems = concatMap processOneCMI combinedMapItems
 where
  ...
  processOneCMI combinedMapItem = potentialLowerBound ++ potentialUpperBound
   where
    potentialLowerBound =
      if declLWB == accessLWB then [] else [(loopVarName, LWB accessLWB)]
    potentialUpperBound =
      if declUPB == accessUPB then [] else [(loopVarName, UPB accessUPB)]
...
buildArrayAccessDimMap loopVarsAndBounds arrayAccessPos = DMap.fromList
  mapItems
 where
  loopVarMap = DMap.fromList $ map
    (\(loopVarName, lb, ub) -> (loopVarName, (lb, ub)))
    loopVarsAndBounds
...
combineWithAnd = buildAstSeq (Bin nullAnno nullSrcSpan (And nullAnno))
                             (NullExpr nullAnno nullSrcSpan)

-- build one condition of the form (<loop variable> <=/>= <const>)
buildLoopGuard :: String -> Bound -> Expr Anno
buildLoopGuard loopVarName bound = ParenthesizedExpr nullAnno
                                                     nullSrcSpan
                                                     comparison
 where
...
getDimensionPositionMap decls = DMap.fromList allItems
  where allItems = concatMap getDimensionPosition decls
