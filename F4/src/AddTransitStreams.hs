...
--
-- If all kernels in a pipeline only read values from a stream (e.g. has it as an input stream but not an output stream) 
-- and the stream is stored in global memory we don't need to transit the stream through the pipeline and can instead
-- emit a memory reader kernel thus saves smart cache space. However, if the stream has ever be output by a kernel
   ...
   where
    ...
    addTransitStreamToKernel ((prodIdx, consumeIdx), stream) kernel
     ...
     where
      ...
      updatedKernel      = kernel
        { inputs  = if inputRequired
                      then buildTransitStream stream : orgIns
                      else orgIns
        , outputs = if outputRequired
                      then buildTransitStream stream : orgOuts
                      else orgOuts
        }
...
getStreamsToTransit kernels unmatchedStreams = streamsToTransit
 where
  streamsToTransit =
    concatMap (searchForStreamProduction kernels) unmatchedStreams
