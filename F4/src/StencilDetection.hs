...
detectStencilsInSubsToBeParallelise srt = updatedSrt
 where
  ...
  updatedSrt =
    foldr (\cur acc -> DMap.insert (subName cur) cur acc) srt withStencilsDetect

detectStencils :: SubRec -> SubRec
detectStencils subrec = removeRedundantStencilNodes
  $ addStencilNodes arraysInSub subrec
  where arraysInSub = map arrayFromDecl $ getArrayDecls (subAst subrec)

removeRedundantStencilNodes :: SubRec -> SubRec
removeRedundantStencilNodes subRec = subRec
  { subAst = everywhere (mkT removeStencilQuery) $ subAst subRec
  }
 where
...
addStencilNodes arrays subRec = subRec
  { subAst = everywhere (mkT addNodeQuery) $ subAst subRec
  }
 where
  addNodeQuery curVal@(OpenCLMap _ _ _ _ _ _ body)
    | (not . null) stencils = OpenCLStencil nullAnno nullSrcSpan stencils curVal
    | otherwise             = curVal
    where stencils = createStencilRecords arrays $ findStencils arrays curVal
...
createStencilRecords arrays stencils = map (uncurry createOneStencilRecord)
                                           arrayDeclUsagePairs
 where
  arrayDeclMapItems =
  ...
  stencilMapItems =
    map (\stencils -> (arrayName $ head stencils, stencils)) stencils
...
createOneStencilRecord arrayDecl stencilArrayAccesses = Stencil
  nullAnno
  dimensions
  (length offsets)
  offsets
...
findStencilAccesses arrays body = arrayAccesses
  where arrayAccesses = everything (++) (mkQ [] (getArrayReads arrays)) body
...
usesIndexVariablesAndConstantOffset
  ...
  -> [Expr Anno]
  -> [Expr Anno]
usesIndexVariablesAndConstantOffset idxVars stencilAccesses = getStencilsOnly
  simpleOpsOnly
 where
  ...
  stencilsUsingLoopVars = getStencilsOnly $ filter
    (\(_, indices) ->
...
stencilToStencilIdxNames sten =
  ...
  , map getNameFromVarName
    $ everything (++) (mkQ [] extractVarNamesFromExpr)
    $ idxVarQuery sten
  )
