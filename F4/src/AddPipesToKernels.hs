  ...
  go newPipeline currentStage = if isJust prevStageWithPipes
   ...
   where
    ...
    getPipe (stream, dest) =
     ...
     where
      valid =
        getArrayNameFromStream stream == getArrayNameFromStream streamFromMap
...
addPipesToPipelineStages prevStage (currKern, currSC, currMA) pipes =
 ...
 where
  ...
  updateWrittenPipes pipe item = item
    ...
    where currentWrittenPipes = writtenPipes item
  originalMap = DMap.fromList $ map (\i -> (name i, i)) $ filter
    (\case
      NullItem -> False
      _        -> True
    )
  ...
  allItems =
    [prevKern]
      ...
      ++ maybeToList currSC
      ++ maybeToList prevSC
  (prevKernName, previousSCName, previousMemAccNames) = if isJust prevStage
...
pickSource dest@SmartCache{} sources = maximumBy
  ...
  ( removeDestFromSources dest
  $ removeComputeKernelOutputsFromSmartCacheSources sources
  )
...
isComputeKernel k = case k of
  Map{}    -> True
  Reduce{} -> True
  _        -> False

isSmartCache s = case s of
  SmartCache{} -> True
  _            -> False

removeComputeKernelOutputsFromSmartCacheSources
  :: [(PipelineItem SharedPipelineData, Stream Anno)]
  -> [(PipelineItem SharedPipelineData, Stream Anno)]
removeComputeKernelOutputsFromSmartCacheSources []      = []
removeComputeKernelOutputsFromSmartCacheSources sources = filter
  (\(item, _) ->
    (  not hasSmartCache
    || (trace
         (  "smart cache order = "
 ...
 where
  ...
  smartCacheOrder =
    (stageNumber . fst . head) $ filter (isSmartCache . fst) sources
...
removePreviousKernelOutputsFromMemWriterSources
  ...
  -> [(PipelineItem SharedPipelineData, Stream Anno)]
removePreviousKernelOutputsFromMemWriterSources []      = []
removePreviousKernelOutputsFromMemWriterSources sources = filter
  (\(item, _) -> stageNumber item == highestOrder)
  kernelSourcesOnly
 where
  kernelSourcesOnly = filter
    (\(item, _) -> case item of
      Map{}    -> True
      Reduce{} -> True
      _        -> False
...
makeMap sourceAndStreams = DMap.fromList mapGroups
 where
  mapItems = map (\(source, s) -> (getArrayNameFromStream s, (source, s)))
                 sourceAndStreams
