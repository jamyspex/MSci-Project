...
detectDriverLoopSize kernels = do
 ...
 where
  ...
  streamNamesAndSizes = map
    (\s -> (getStreamName s, (calculateStreamSize . getStreamDimensions) s))
    allStreams
  (largestStreamName, largestStreamSize) = maximumBy
...
updatePipelineSharedData (lowerBound, upperBound) = map updateStage
 where
  ...
  updateStage (kernel, smartCache, memAccess) =
   ...
   where
    sharedData =
      SPD {driverLoopUpperBound = upperBound, driverLoopLowerBound = lowerBound}
    ...
    updateItem item = item { sharedData = sharedData }
