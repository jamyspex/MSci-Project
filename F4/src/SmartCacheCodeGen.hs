...
buildSmartCacheBody (mainLoopLowerBound, mainLoopUpperBound) smc@SmartCache {..} =
  ( for mainLoopVarName mainLoopLowerBound (var mainLoopBoundName) mainLoopBody
  ...
  where
    ...
    mainLoopBoundVal = mainLoopUpperBound + maxPosOffset
...
generatePipeReads SmartCache {..} =
  concatMap (uncurry $ generatePipeRead assignmentIdx) toProcess
  where
    toProcess =
      map
