...
removeIntentFromLoopVars loopVars kernel = kernel { body = newKernelCode }
 where
  ...
  updatedBlock    = Block blockAnno
                          uses
                          implicit
                          blockSrcSpan
                          (declNode withIntentRemovedFromLoopVarDecls)
                          fortran
  withIntentRemovedFromLoopVarDecls = map
    (\d -> if (getNameFromVarName . getVarNameG) d `elem` loopVars
      then removeIntentAttr d
      else d
    )
    allDecls
  removeIntentAttr :: Decl Anno -> Decl Anno
  removeIntentAttr d@(Decl an srcSpn expr fortranType) = Decl an
                                                              srcSpn
                                                              expr
                                                              updatedType
   where
    updatedType = BaseType nullAnno baseType attrsWithIntentsRemoved kind len
    ...
    attrsWithIntentsRemoved = filter
      (\case
        (Intent _ _) -> False
        _            -> True
      )
...
addDerivationCodeToKernelBody dervCode kernel = kernel
  { driverLoopVariableName = loopVarName
  , body                   = withDervCodeInserted
  }
 where
  kernelCode           = body kernel
  withDervCodeInserted = everywhere (mkT addDervCodeQuery) kernelCode
  addDervCodeQuery fortran = case fortran of
    (OpenCLMap _ _ read written nestVars enclosedVars body) -> OpenCLMap
      nullAnno
      nullSrcSpan
      read
      written
...
emitLoopVarDerivationCode allLoopVarInfo = block
...
emitOneLoopVarDerivation driverLoopIdxName allLoopVarInfo (nestLevel, loopVarName, (lwb, upb))
  ...
  where moduloDivisor = multiplyBounds nestLevel allLoopVarInfo

multiplyBounds nestLevel allLoopVarInfo =
  product $ map (\(_, _, (lwb, upb)) -> upb - lwb + 1) $ drop (nestLevel + 1)
                                                              allLoopVarInfo
...
getLoopVarToRange kernelCode = map
  ( (\(loopVarName, dims) ->
...
allStreamsSameSize kernels = do
  ...
  return valid
 where
  printSize s = putStrLn
    ("stream = " ++ getStreamName s ++ " dims = " ++ show
      (getStreamDimensions s)
    )
