...
outputDirectoryParser = strOption
  (  long "outputDir"
  <> short 'o'
  <> value "./out"
...
mainSubParser = strOption
  (long "main" <> short 'm' <> metavar "<MAIN FILE>" <> help
    "Main file with time step loop."
  )

iterationOrderParser :: Parser Bool
iterationOrderParser =
  switch (long "useCItOrder" <> short 'i' <> help "Use C iteration order.")

subsForFPGAParser :: Parser [String]
subsForFPGAParser = many
  (strOption
    (long "offload" <> short 'o' <> metavar "<FILE FOR OFFLOAD>" <> help
      "Files to offload to FPGA"
    )
  )

otherSubsParser :: Parser [String]
otherSubsParser = many
  (strOption
    (  long "othersub"
    <> short 's'
...
cppDefinesParser = option
  cppNameValueReader
  (long "cppDefine" <> metavar "<CPP DEFINES>" <> short 'D' <> value [] <> help
    "CPP #defines, format: NAME[=VALUE]"
  )
